...
if __name__ == '__main__':
    ...
    pos_pub = rospy.Publisher("pos_gps",PointStamped,queue_size=1)
    port = rospy.get_param("~GPS_PORT","/dev/ttyACM0")
    print(port)
    ...
    while isrunning:
        ...
        try:
            if "GGA" in RoverMessege:
                ...
                if(len(lat_str)==12): 
                    deg_lat = float(lat_str[0:2])
                    min_lat = float(lat_str[3:])   
                elif(len(lat_str)==13): 
                    deg_lat = float(lat_str[0:3])
                    min_lat = float(lat_str[3:])    
                
                if(len(lon_str)==13): 
                    deg_lon = float(lon_str[0:3])
                    min_lon = float(lon_str[3:])    
                elif(len(lon_str)==12): 
                    deg_lon = float(lon_str[0:2])
                    min_lon = float(lon_str[2:]) 
                ...
                if(isReady):
                    pos.point.x = (pos_data[0] - 460000)/100
                    pos.point.y = (pos_data[1] - 383000)/100
