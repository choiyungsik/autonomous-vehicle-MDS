...
void callback(const sensor_msgs::NavSatFixConstPtr& fix) {
  ...
  if (odom_pub) {
    ...
    yaw_ = yaw+43.2;
    //yaw_ = yaw*0.9 - 85 * (3.14/180); // because our imu is not right hand rule and align
    ...
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "map", "odom"));  // gps odom only with imu_yaw
